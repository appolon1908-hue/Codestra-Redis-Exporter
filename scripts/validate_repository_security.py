#!/usr/bin/env python3
"""Validate Codestra Redis Exporter protected source authority."""
        ...
        for index, word in enumerate(words):
            ...
            for candidate in words[command_index + 1 :]:
                ...
                if refspec == "HEAD:refs/heads/${SYNC_BRANCH}":
                    continue
                if "$" in refspec or "`" in refspec or any(
                    marker in refspec for marker in ("*", "?", "[")
                ):
                    raise ValueError("protected_branch_sync_forbidden")
...
def validate_upstream(source: dict, lock: dict) -> None:
    expected = {
        ...
        "codestra_repository": "appolon1908-hue/Codestra-Redis-Exporter",
        "upstream_repository": "oliver006/redis_exporter",
        "upstream_clone_url": "https://github.com/oliver006/redis_exporter.git",
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"REDIS_EXPORTER_SOURCE_SECURITY=FAIL ERROR={error}") from error
    print("REDIS_EXPORTER_SOURCE_SECURITY=PASS")
